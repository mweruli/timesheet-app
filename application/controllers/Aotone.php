...
//                print_array($array_user);
    ...
    public function manualload_monthly($date, $serialnumber) {
        ...
        $this->db->trans_start();
        foreach ($newpinsdata as $datadate) {
            $them = $this->availablemonthlydata($datadate ['pin'], $datadate ['datelike'], $datadate ['serialnumber']);
//            print_array($them);
            $getmearray1 = $this->fullfledgedreportrecord($datadate ['datelike'], $them);
            // $result = array();
            array_walk($getmearray1, function ($value, $key) use (&$result) {
                unset($value ['exactdate']);
                unset($value ['dateinsertserver']);
...
//            print_array($firstCharacter);
    ...
    public function manualcomit($login, $logout, $transid) {
        ...
        $manualentry ['off'] = $this->off_caluculus(getfirstday_of_week($data [0] ['firstdayofweek']), $data [0] ['dateclock'], $this->tothrs($timerange, $shift_def [0] ['lunchmin']), $data [0] ['pin']);
        // print_array($manualentry);
