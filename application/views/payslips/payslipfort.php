<div class="panel-body">
    <form class="form-horizontal" role="form" method="post"
          name="payslip_form" id="payslip_form" novalidate>
        <div class="form-group">
            <label class="col-sm-1 control-label"> Branch </label>
            <div class="col-sm-5">
                <select id="branch_startpay" name="branch_startpay"
                        class="form-control search-select" onChange="selectuserbranchpay();">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-5 date">
                <div class="input-group input-append date" id="date_mincardxpay">
                    <input type="text" id="date_mincardx_actualidp" name="date_mincardxpay"
            ...
            <div class="col-sm-5 date">
                <div class="input-group input-append date" id="date_maxncardxpay">
                    <input type="text" class="form-control"
                           id="date_maxncardxpay_actualidpay" name="date_maxncardxpay"> <span
                           class="input-group-addon add-on"><span
                            class="glyphicon glyphicon-calendar"></span></span>
                </div>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="col-sm-4">
                <select id="employeenumberminxpay" name="employeenumberminxpay"
            ...
            <label class="col-sm-1 control-label"> To </label>
            <div class="col-sm-5">
                <select id="employeenumbermaxxpay" name="employeenumbermaxxpay"
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu dropdown-light">
                        <li><a href="#" id="exportpdf"
                               onclick="printJS({printable: 'payslipcard_attendence_card', type: 'html', maxWidth: '1000', showModal: true, repeatTableHeader: true, honorMarginPadding: false, targetStyles: ['*']})">
                                Save as PDF </a></li>
                    </ul>
                </div>
        ...
        <div class="form-group" id="tablerespdividpayslip"></div>
        <style>
            @media print {
...
<script>
    var loadpayslipt = "<?php echo base_url("rfortnight/loadpayslip"); ?>";
    function selectuserbranchpay() {
        var e = document.getElementById("branch_startpay");
        $('#employeenumberminxpay').find('option:not(:first)').remove();
        $('#employeenumbermaxxpay').find('option:not(:first)').remove();
//         $('#employeenumberminxpay').get(0).selectedIndex = 0;
        //To Extraxt
...
//         console.log(mile_stones2);
        var $dropdown = $("#employeenumberminxpay");
        var $dropdown1 = $("#employeenumbermaxxpay");
...
//     	selectuserbranchpay();
        $("#employeenumberminxpay").val($("#employeenumberminxpay option:first").val());
        $("#employeenumbermaxxpay").val($("#employeenumbermaxxpay option:first").val());
        var date = new Date(), y = date.getFullYear(), m = date.getMonth();
        var firstDay = new Date(y, m, 1);
        var lastDay = new Date(y, m + 1, 0);
        var dateone = formatDate(firstDay, 'yyyy-mm-dd');
        var datetwo = formatDate(lastDay, 'yyyy-mm-dd');
        $('#date_mincardx_actualidp').val(dateone);
        $('#date_maxncardxpay_actualidpay').val(datetwo);
// console.log(date);
        $('#date_mincardxpay')
                .datepicker({
                    format: 'yyyy-mm-dd'
            ...
            $('#payslip_form').formValidation('revalidateField', 'date_mincardxpay');
        });
        $('#date_maxncardxpay')
                .datepicker({
                    format: 'yyyy-mm-dd'
            ...
            // Revalidate the date field
            $('#payslip_form').formValidation('revalidateField', 'date_maxncardxpay');
        });
    });
    $("#payslip_form").formValidation({
        framework: 'bootstrap',
        icon: {
...
//            console.log(response);
            if (response.status === 1) {
                ...
                $("#tablerespdividpayslip").html(response.data);
            } else {
                $.notify(response.report, "error");
                $("#tablerespdividpayslip").html('');
            }
        });
    });
