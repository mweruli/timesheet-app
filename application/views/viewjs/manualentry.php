<script>
        ...
        $
            .ajax({
                ...
                success: function(response) {
                    var en = JSON.parse(response);
                    // var en = JSON.stringify(response);
                    if (en.status == 0) {
                        $.notify(en.report, "info");
                    } else {
                        $.notify(en.report, "error");
                    }
    ...
    DatabaseGrid.prototype.fetchGrid = function(tablename) {
        ...
        //    console.log('joash');
        console.log("<?= base_url() ?>posts/loadallowances/" + tablename);
        this.editableGrid.loadJSON("<?= base_url() ?>posts/loadallowances/" + tablename);
    };
    DatabaseGrid.prototype.addRow = function(tablename, type, formdata, contentid) {
        //    console.log(id + "njosvu joshua");
        $.ajax({
            url: '<?= base_url() ?>posts/addalloance',
            type: type,
            dataType: "json",
            data: formdata,
            processData: false,
