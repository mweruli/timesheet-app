...
function cke_create_instance($data = array()) {
    ...
    if (isset($data['config'])) {
        foreach ($data['config'] as $k => $v) {
            ...
            $num_var = array_keys($data['config']);
            if ($k !== end($num_var)) {
                $return .= ",";
            }
        }
...
function display_ckeditor($data = array()) {
    ...
    if (isset($data['styles'])) {
        ...
        foreach ($data['styles'] as $k => $v) {
            ...
            $return .= '} }';
            $walah_2 = array_keys($data['styles']);
            if ($k !== end($walah_2)) {
                $return .= ',';
            }
        }
    ...
    foreach ($data as $key) {
        if (is_array($key)) {
            ...
            foreach ($key as $string) {
                $return .= "'" . $string . "'";
                $walah_x = array_values($key);
                if ($string != end($walah_x))
                    $return .= ",";
            }
            $return .= "]";
        }
