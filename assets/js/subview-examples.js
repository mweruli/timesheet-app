	...
	var showContributors = function() {
		...
		for ( var i = 0; i < contributors.length; i++) {
			...
			LISTING_RESULTS[i] = new Array(item.avatar, item.firstName + ' ' + item.lastName, item.email, item.id_gender, item.permits, i);
		}
	...
	var editContributor = function(el) {
		...
		if ( typeof el == "undefined") {
			...
			$('.contributor-id_gender').iCheck('uncheck');
			...
			$(".contributor-id_gender[value='" + contributors[el].id_gender + "']").iCheck('check');
	...
	var runContributorsFormValidation = function(el) {
		...
		formContributor.validate({
			...
			messages : {
				...
				id_gender : "Please check a id_gender!"
			},
			invalidHandler : function(event, validator) {//display error alert on form submit
